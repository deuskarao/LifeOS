...
export async function POST(req: NextRequest) {
  try {
    ...
    const { name, plate, brand, model, year, fuelType, currentKm, currentValue, color, notes } = body as Record<string, string | number | null>
    ...
    const item = await store.create('vehicles', user.id, {
      ...
      currentKm: Number(currentKm) || 0, currentValue: Number(currentValue) || 0, color: color as string | null,
